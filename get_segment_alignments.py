...
def write_parallel_index_files(conf):
    ...
    for entry in os.scandir(lang_pair_work_directory):
        if entry.is_file() and entry.name.endswith('_{}.snt.aligned'.format(conf['source_language'])):
            ...
            idx_docs = [os.path.join(lang_pair_alignment_index_directory,
                                     '{}.{}.idx'.format(pair_title, conf['source_language'])),
