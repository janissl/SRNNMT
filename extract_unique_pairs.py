...
def write_unique_file_pair(conf):
    lang_pair = '{}-{}'.format(conf['source_language'], conf['target_language'])
    path_base = os.path.join(conf['output_data_directory'], conf['corpus_title'])

    all_source_path = '{}.{}.{}'.format(path_base, lang_pair, conf['source_language'])
    all_target_path = '{}.{}.{}'.format(path_base, lang_pair, conf['target_language'])

    unique_source_path = '{}.unique.{}.{}'.format(path_base, lang_pair, conf['source_language'])
    unique_target_path = '{}.unique.{}.{}'.format(path_base, lang_pair, conf['target_language'])
