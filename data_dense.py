...
class Matrices:
    def __init__(self, minibatch_size, max_sent_len, ngrams):
        self.source_ngrams = dict()  # { N -> matrix}
        self.target_ngrams = dict()  # { N -> matrix}
...
class Vocabularies(object):
    def __init__(self, ngrams):
        self.source_ngrams = dict()  # {order -> {"<MASK>":0,"<UNK>":1}} # source language ngrams
        self.target_ngrams = dict()  # {order -> {"<MASK>":0,"<UNK>":1}} # target language characters
    ...
    def get_id(self, label, dict_obj):
        if self.trainable:
            return dict_obj.setdefault(label, len(dict_obj))  # auto-grows
        else:
            return dict_obj.get(label, dict_obj["<UNK>"])


def iter_data(training_source, training_target, max_pairs=None):
    counter = 0

    with open(training_source, encoding='utf-8') as src, open(training_target, encoding='utf-8') as trg:
        for src_line, trg_line in zip(src, trg):
            # if 5 <= len(src_line.strip().split()) <= 30 and \
            #         5 <= len(trg_line.strip().split()) <= 30: # sentence length must be between 5 and 30 tokens
            if True:
                yield src_line, trg_line
                counter += 1

                if max_pairs is not None and counter >= max_pairs > 0:
                    break
...
class InfiniteDataIterator:
    ...
    def __iter__(self):
        ...
        """
        # [(0,0),(1,1),(2,2)...]  #indices of source, target
        positive_indices = list(zip(range(len(self.data)), range(len(self.data))))
        counter = 0
        ...
        while True:
            ...
            for src_idx, trg_idx in all_examples:
                # index where I should take the source sentence, index where I should take the target sentence
...
def read_vocabularies(voc_fname, training_source, training_target, force_rebuild, ngrams):
    ...
    if force_rebuild or not os.path.exists(voc_fname):
        ...
        for (sent_src, sent_target), _ in InfiniteDataIterator(training_source,
                                                               training_target,
                                                               max_iterations=1,
                                                               max_pairs=1000000):
            counter += 1
...
def fill_batch(minibatch_size, max_sent_len, vs, data_iterator, ngrams):
    ...
    batchsize, max_sentence_len = ms.source_ngrams[ngrams[0]].shape
    row = 0
...
#     vs = read_vocabularies('bidirectional-ngrams',
#                            "data/JRC-Acquis.en-fi.fi",
#                            "data/JRC-Acquis.en-fi.en",
#                            False,
#                            (3, 4))
#     vs.trainable = False
