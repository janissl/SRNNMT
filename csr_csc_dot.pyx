...
cdef extern void fast_dot(idx_type m1_start,
                          idx_type m1_slice_len,
                          idx_type *m1_row_ptr,
                          idx_type *m1_indices,
                          val_type *m1_data,
                          idx_type *m2_col_ptr,
...
def csr_csc_dot_f(start, slice, m1, m2, np.ndarray[np.float32_t, ndim=2, mode="c"] out):
    ...
    if m1.shape[0] != out.shape[0] and m2.shape[0] != out.shape[1]:
       raise ValueError(
           "M1 shape[0]={} should match out shape[0]={} and M2 shape[0]={} should match out shape[1]={}".format(
               m1.shape[0], out.shape[0], m2.shape[0], out.shape[1]))
